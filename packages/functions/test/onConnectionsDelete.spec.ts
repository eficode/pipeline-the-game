...
describe("onConnectionsDelete", () => {
  ...
  it("should decrease connection count correctly", async () => {
    ...
    await admin
      .firestore()
      .doc(`${FirebaseCollection.RTDBInstances}/${rtdbInstances[0].id}`)
      .set({ connectionsCount: 1 }, { merge: true });
    await admin
      .firestore()
      .doc(`${FirebaseCollection.Games}/${gameId}`)
  ...
  it("should unlock cards related to the userId correctly", async () => {
    ...
    await admin
      ...
      .doc(`${FirebaseCollection.RTDBInstances}/${rtdbInstances[0].id}`)
      .set({ connectionsCount: 1 }, { merge: true });
    await admin.firestore().doc(`${FirebaseCollection.Games}/${gameId}`).set({
  ...
  it("should not touch lastPlayerDisconnectedAt if there are still players online", async () => {
    ...
    await admin
      ...
      .push({ updatedAt: 124342 });
    await admin
      .app()
      .database(
        `https://pipeline-game-development-default-rtdb.europe-west1.firebasedatabase.app`
      )
      .ref(`${RTDBPaths.Connections}/${gameId}/randomUserId`)
      .push({ updatedAt: 124342 });
    ...
    const gameData = (
      await admin.firestore().doc(`${FirebaseCollection.Games}/${gameId}`).get()
    ).data() as Game;
