...
describe("scheduledMoveGame", () => {
  ...
  it("should not move game with recent lastPlayerDisconnectedAt", async () => {
    await admin.firestore().doc(`${FirebaseCollection.Games}/gameId1`).set({
      lastPlayerDisconnectedAt: admin.firestore.FieldValue.serverTimestamp(),
    });
    await admin
      .app()
      .database(
      ...
      .ref(`${RTDBPaths.Games}/gameId1`)
      .set({ something: "" });

    await assert.isFulfilled(functions.scheduledMoveGame());

    const rtdbGameSnap = await admin
      .app()
      .database(
        `https://pipeline-game-development-default-rtdb.europe-west1.firebasedatabase.app`
      )
  ...
  it("should not move game with null lastPlayerDisconnectedAt", async () => {
    await admin
      .firestore()
      .doc(`${FirebaseCollection.Games}/gameId1`)
  ...
  it("should move game and cards if lastPlayerDisconnectedAt is < now - 24h ", async () => {
    await admin
      ...
      .set({
        ...
        lastPlayerDisconnectedAt: new admin.firestore.Timestamp(
  ...
  it("should move game if lastPlayerDisconnectedAt is < now - 24h preserving createdAt", async () => {
    ...
    const gameData = (
      await admin.firestore().doc(`${FirebaseCollection.Games}/gameId1`).get()
    ).data() as Game;

    expect((gameData.createdAt as admin.firestore.Timestamp).toMillis()).eq(
      new admin.firestore.Timestamp(14424, 1424).toMillis()
    );
  });
  ...
  it("should move game if lastPlayerDisconnectedAt is < now - 24h updating movedAt", async () => {
    ...
    ).movedAt as admin.firestore.Timestamp;
    await admin
      .app()
      .database(
        `https://pipeline-game-development-default-rtdb.europe-west1.firebasedatabase.app`
    ...
    expect(
      (gameData.movedAt as admin.firestore.Timestamp).toMillis() >
        oldMovedAt.toMillis()
    ).eq(true);
  });
