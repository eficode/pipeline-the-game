...
describe("Dynamic data game roles read", () => {

  it("should allow game roles read if not authenticated", async () => {
    const db = getAuthedFirestore(PROJECT_ID, undefined);
    const gameRolesRef = db.collection(FirebaseCollection.DynamicData).doc(FirebaseDoc.GameRoles);
    await firebase.assertSucceeds(gameRolesRef.get());
  });

  it("should allow game roles read if authenticated but email not verified", async () => {
    const userUID = 'id1';
  ...
  it("should allow game roles read if authenticated and email verified", async () => {
    const userUID = 'id1';
    const email = 'test@email.com';
